...
class IndexTest : public TestBase, public ::testing::WithParamInterface<std::string> {
protected:
    void SetUp() override {
        TestBase::SetUp();
        ...
        std::string query = "CREATE SPACE index_test_space(partition_num=1, replica_factor=1)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);

        query = "USE index_test_space";
    ...
    void TearDown() override {
        cpp2::ExecutionResponse resp;
        std::string query = "DROP SPACE index_test_space";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
        TestBase::TearDown();
    }
...
INSTANTIATE_TEST_CASE_P(IndexQueryTest, IndexTest, ::testing::Values("", "UNIQUE"));

TEST_P(IndexTest, TagIndex) {
    {
        cpp2::ExecutionResponse resp;
        auto query = "CREATE TAG person(name string, age int, gender string, email string)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    ...
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE TAG " + GetParam()
            +  " INDEX single_person_index ON person(name)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    }
    // Tag not exist
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE TAG " + GetParam()
            + " INDEX single_person_index ON student(name)";
        auto code = client_->execute(query, resp);
    ...
    // Property not exist
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE TAG " + GetParam()
            + " INDEX single_person_index ON person(phone)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::E_EXECUTION_ERROR, code);
    }
    // Property is empty
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE TAG " + GetParam() + " INDEX single_person_index ON person()";
        auto code = client_->execute(query, resp);
    ...
    // Single Tag Multi Field
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE TAG " + GetParam()
            + " INDEX multi_person_index ON person(name, email)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    }
    {
    ...
    }
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE TAG " + GetParam()
            + " INDEX duplicate_index ON person(name, name)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::E_EXECUTION_ERROR, code);
    }
    // Describe Tag Index
    ...
    // Show Create Tag Indexes
    {
        ...
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
        std::string createTagIndex;
        if (GetParam() == "") {
            createTagIndex = "CREATE TAG INDEX multi_person_index ON person(name, email)";
        } else if (GetParam() == "UNIQUE") {
            createTagIndex = "CREATE TAG " + GetParam()
                + " INDEX multi_person_index ON person(name, email)";
        } else {
            ASSERT_TRUE(false) << "Invalid GetParam(): " << GetParam();
        }
...
TEST_P(IndexTest, EdgeIndex) {
    {
        cpp2::ExecutionResponse resp;
        auto query = "CREATE EDGE friend(degree string, start_time int)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    ...
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE EDGE " + GetParam()
            + " INDEX single_friend_index ON friend(degree)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    }
    // Edge not exist
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE EDGE " + GetParam()
            + " INDEX single_friend_index ON friendship(name)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::E_EXECUTION_ERROR, code);
    }
    // Property not exist
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE EDGE " + GetParam()
            + " INDEX single_friend_index ON friend(startTime)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::E_EXECUTION_ERROR, code);
    }
    // Property is empty
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE EDGE " + GetParam() + " INDEX single_friend_index ON friend()";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::E_SYNTAX_ERROR, code);
    }
    // Single EDGE Multi Field
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE EDGE " + GetParam()
            + " INDEX multi_friend_index ON friend(degree, start_time)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    }
    {
    ...
    }
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE EDGE " + GetParam()
            + " INDEX duplicate_index ON friend(degree, degree)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::E_EXECUTION_ERROR, code);
    }
    // Describe Edge Index
    ...
    // Show Create Edge Index
    {
        ...
        std::string createTagIndex;
        if (GetParam() == "") {
            createTagIndex = "CREATE EDGE INDEX multi_friend_index ON "
                                        "friend(degree, start_time)";
        } else if (GetParam() == "UNIQUE") {
            createTagIndex = "CREATE EDGE " + GetParam() + " INDEX multi_friend_index ON "
                                        "friend(degree, start_time)";
        } else {
            ASSERT_TRUE(false) << "Invalid GetParam(): " << GetParam();
        }
...
TEST_P(IndexTest, TagIndexTTL) {
    {
        cpp2::ExecutionResponse resp;
        auto query = "CREATE TAG person_ttl(name string, age int, gender int, email string)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    }
    // Single Tag Single Field
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE TAG " + GetParam()
            + " INDEX single_person_ttl_index ON person_ttl(age)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    }
    ...
    // Tag with ttl to create index on ttl col, failed
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE TAG " + GetParam()
            + " INDEX single_person_ttl_index_second ON person_ttl(gender)";
        auto code = client_->execute(query, resp);
        ASSERT_NE(cpp2::ErrorCode::SUCCEEDED, code);
    }
    // Tag with ttl to create index on not ttl col, failed
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE TAG " + GetParam()
            + " INDEX single_person_ttl_index_second ON person_ttl(age)";
        auto code = client_->execute(query, resp);
        ASSERT_NE(cpp2::ErrorCode::SUCCEEDED, code);
    }
    // Drop ttl propery
    ...
    // Tag with ttl cannot create index on not ttl col
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE TAG " + GetParam()
            + " INDEX person_ttl_2_index ON person_ttl_2(name)";
        auto code = client_->execute(query, resp);
        ASSERT_NE(cpp2::ErrorCode::SUCCEEDED, code);
    }
    // Tag with ttl cannot create index on ttl col
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE TAG " + GetParam()
            + " INDEX person_ttl_2_index ON person_ttl_2(age)";
        auto code = client_->execute(query, resp);
        ASSERT_NE(cpp2::ErrorCode::SUCCEEDED, code);
    }
    // Drop ttl col
...
TEST_P(IndexTest, EdgeIndexTTL) {
    {
        cpp2::ExecutionResponse resp;
        auto query = "CREATE EDGE friend_ttl(degree int, start_time int)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    }
    // Single Edge Single Field
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE EDGE " + GetParam()
            + " INDEX single_friend_ttl_index ON friend_ttl(start_time)";
        auto code = client_->execute(query, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    }
    ...
    // Edge with ttl to create index on ttl col, failed
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE EDGE " + GetParam()
            + " INDEX friend_ttl_index_second ON friend_ttl(degree)";
        auto code = client_->execute(query, resp);
        ASSERT_NE(cpp2::ErrorCode::SUCCEEDED, code);
    }
    // Edge with ttl to create index on no ttl col, failed
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE EDGE " + GetParam()
            + " INDEX friend_ttl_index_second ON friend_ttl(start_time)";
        auto code = client_->execute(query, resp);
        ASSERT_NE(cpp2::ErrorCode::SUCCEEDED, code);
    }
    // Drop ttl propery
    ...
    // Edge with ttl cannot create index on not ttl col
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE EDGE " + GetParam()
            + " INDEX friend_ttl_index_2 ON friend_ttl_2(degree)";
        auto code = client_->execute(query, resp);
        ASSERT_NE(cpp2::ErrorCode::SUCCEEDED, code);
    }
    // Edge with ttl cannot create index on ttl col
    {
        cpp2::ExecutionResponse resp;
        std::string query = "CREATE EDGE " + GetParam()
            + " INDEX friend_ttl_index_2 ON friend_ttl_2(start_time)";
        auto code = client_->execute(query, resp);
        ASSERT_NE(cpp2::ErrorCode::SUCCEEDED, code);
    }
    // Drop ttl col
