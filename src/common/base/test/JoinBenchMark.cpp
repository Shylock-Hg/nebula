...
BENCHMARK_RELATIVE(F14_1k, iters) {
  constexpr std::size_t kData = 1000;
  for (auto i = 0u; i < iters; i++) {
    folly::F14FastMap<std::size_t, std::size_t> t(kData);
    t.reserve(kData);
    for (std::size_t j = 0; j < kData; j++) {
      t[j] = j;
    }
...
BENCHMARK_RELATIVE(F14_10k, iters) {
  constexpr std::size_t kData = 10000;
  for (auto i = 0u; i < iters; i++) {
    folly::F14FastMap<std::size_t, std::size_t> t;
    t.reserve(kData);
    for (std::size_t j = 0; j < kData; j++) {
      t[j] = j;
    }
...
BENCHMARK_RELATIVE(F14_10k_value_int, iters) {
  constexpr int64_t kData = 10000;
  for (auto i = 0u; i < iters; i++) {
    folly::F14FastMap<Value, std::vector<const List*>> t;
    t.reserve(kData);
    for (int64_t j = 0; j < kData; j++) {
      t[Value(j)].emplace_back(nullptr);
    }
...
BENCHMARK_RELATIVE(F14_10k_value_string, iters) {
  constexpr int64_t kData = 10000;
  for (auto i = 0u; i < iters; i++) {
    folly::F14FastMap<Value, std::vector<const List*>> t;
    t.reserve(kData);
...
BENCHMARK(std_10k_list, iters) {
  constexpr int64_t kData = 10000;
  for (auto i = 0u; i < iters; i++) {
    std::unordered_map<List, std::vector<const List*>> t;
    t.reserve(kData);
    for (int64_t j = 0; j < kData; j++) {
      t[List({Value(j), Value(std::to_string(j))})].emplace_back(nullptr);
    }
    for (int64_t j = 0; j < kData; j++) {
      auto found = t.find(List({Value(j), Value(std::to_string(j))}));
      folly::doNotOptimizeAway(found);
    }
  }
}

BENCHMARK_RELATIVE(F14_10k_list, iters) {
  constexpr int64_t kData = 10000;
  for (auto i = 0u; i < iters; i++) {
    folly::F14FastMap<Value, std::vector<const List*>> t;
    t.reserve(kData);
...
BENCHMARK_RELATIVE(JoinHashTable_10k_list, iters) {
  constexpr int64_t kData = 10000;
  for (auto i = 0u; i < iters; i++) {
    JoinHashTable<Value, std::vector<const List*>> t(kData);
    for (int64_t j = 0; j < kData; j++) {
